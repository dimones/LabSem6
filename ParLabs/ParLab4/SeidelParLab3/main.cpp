...
float *opencl_main(float **a,float *b, int n,int &time){
    ...
    cout << "group size: " << global_work_size << endl;
...
//    !converge(x, p, n)
    do
    {
        ...
        clEnqueueWriteBuffer(command_queue,xbuff,CL_TRUE,0,sizeof(float) * n, x,0,NULL,NULL);
        
        cl_mem pbuff = clCreateBuffer(context, CL_MEM_READ_ONLY, n * sizeof(float), NULL, &ret);
        clEnqueueWriteBuffer(command_queue,pbuff,CL_TRUE,0,sizeof(float) * n, p,0,NULL,NULL);
        
        float temp_norm = 0;
        clSetKernelArg(kernel, 0, sizeof(cl_mem), (void*)&xbuff);
        
        clSetKernelArg(kernel, 1, sizeof(cl_mem), (void*)&pbuff);
...
//        cout << "create buffer norm: " << ret << endl;
        ...
        clEnqueueWriteBuffer(command_queue,normbuff,CL_TRUE,0,sizeof(float), &temp_norm,0,NULL,NULL);
...
//    }
    int time = 0;
    float *out_gpu = opencl_main(a, b, n,time);
    cout << "GPU time: " << time << endl;
    cout << "Boost: " << (double)(((double)cpu_time)/((double)time)) << endl;
//    for(int i = 0;i < n; i++){
//        cout << "x" << i+1 << "  : " << out[i] << endl;
//    }
    return 0;
